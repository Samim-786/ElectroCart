...
export default function CreateStore() {
            ...
            if(['approved','rejected','pending'].includes(data.status)) {
                ...
                switch(data.status){
                    case "approved":
                        ...
                        break;
                    case "rejected":
                        setMessage("Your store request has been rejected for some reason,contact the admin for more details")
                        break;    
                    case "pending":
    ...
    const onSubmitHandler = async (e) => {
        e.preventDefault()
        if (!user) {
            return toast("You have to log in to submit")
        }
        try {
            const token = await getToken();
            ...
            if (data.message) {
                ...
                toast.error(`Store status: ${data.status}`);
            }
    ...
    if (!user) {
        return (
            <div className="min-h-[80vh] mx-6 flex items-center justify-center text-slate-600">
                <h1 className="text-2xl sm:text-4xl font-s">Please <span className="text-slate-700">Log in to continue</span></h1>
