        ...
        const checkStore = async () => {
            if (isSignedIn && user) {
                try {
                    ...
                    const { data } = await axios.get("/api/store/check", {
    ...
    const linkSections = [
        ...
        },
        {
            ...
            links: [
                ...
                { 
                    text: loading ? "Loading..." : (hasStore ? "Go to Your Store" : "Create Your Store"), 
                    path: hasStore ? '/store' : '/create-store', 
                    icon: null 
                },
            ]
        },
        {
            title: "CONTACT",
            links: [
                { text: "+91-9641397312", path: '/', icon: PhoneIcon },
                { text: "electrocart123@gmail.com", path: '/', icon: MailIcon },
                { text: "Jadavpur,Kolkata, 700032", path: '/', icon: MapPinIcon }
            ]
        }
    ];
        ...
        <footer className="mx-6 bg-white">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-slate-500/30 text-slate-500">
                    <div>
                        <Link href="/" className="text-4xl font-semibold text-slate-700">
                            <span className="text-green-600">electro</span>cart<span className="text-green-600 text-5xl leading-0">.</span>
                        </Link>
                        <p className="max-w-[410px] mt-6 text-sm">Welcome to electrocart, your ultimate destination for the latest and smartest gadgets. From smartphones and smartwatches to essential accessories, we bring you the best in innovation — all in one place.</p>
                ...
                <p className="py-4 text-sm text-slate-500">
                    Copyright 2026 © electrocart All Right Reserved.
                </p>
            </div>
        </footer>
    );
